...
namespace Vsite.Oom.Battleship.Model.Tests
{
    ...
    public class SurroundingTargetSelectorTests
    {
        ...
        public void NextReturnsOneOfThreeSquaresAroundSquare3x4AfterSquare3x5IsMissed()
        {
            ...
            var candidates = new SquareCoordinate[] { new(2, 4), new(4, 4), new(3, 3)};  // Kolekcija koordinata koje očekujem da će biti.
            Assert.IsTrue(candidates.Contains(new SquareCoordinate(target.Row, target.Column)));
        }

       [TestMethod]
        public void NextReturnsOneOfTwoSquaresAroundSquare3x4AfterSquare3x5And3x3AreMissed()
        {
            var grid = new ShotsGrid(10, 10);
            var squareHit = grid.Squares.FirstOrDefault(s => s.Row == 3 && s.Column == 4);
            squareHit!.ChangeState(SquareState.Hit);
            ...
            var candidates = new SquareCoordinate[] { new(2, 4), new(4, 4)};  // Kolekcija koordinata koje očekujem da će biti.
            Assert.IsTrue(candidates.Contains(new SquareCoordinate(target.Row, target.Column)));
        }

       [TestMethod]
        public void NextReturnsTheOnlySqureAroundSquare3x4AfterSquare3x5And3x3And2x4AreMissed()
        {
            var grid = new ShotsGrid(10, 10);
            var squareHit = grid.Squares.FirstOrDefault(s => s.Row == 3 && s.Column == 4);
            squareHit!.ChangeState(SquareState.Hit);
